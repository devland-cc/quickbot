    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                Text("The Quickbot server is unreachable. Turn it on from the Quickbot menu bar switch, or update the endpoint in Settings.")
                    .lineLimit(nil)
                    .fontWeight(.medium)
                    .font(.system(size: 14))
            ...
            ActivityIndicatorView(isVisible: .constant(true), type: .growingCircle)
                ...
                .accessibilityLabel(Text("Checking server connection"))
